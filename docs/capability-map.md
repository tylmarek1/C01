...
| Dark mode / theming | Strong | `lib/theme.tsx` (`ThemeProvider`/`useTheme`, mirrors `lib/i18n.tsx`'s shape) + `components/shared/theme-toggle.tsx`. Detected from `prefers-color-scheme`, persisted, no FOUC (`useLayoutEffect`). Because the design system already routed color through CSS custom properties, one `.dark` token block in `index.css` re-themes nearly the whole app; the ~13 spots using a raw Tailwind palette color instead got explicit `dark:` pairs. `color-scheme` set too, so browser-native chrome (date-input glyph, scrollbars) follows the theme — found and fixed as a real gap the sweep surfaced. Verified across all 4 personas and ~25 pages/states, zero console errors, light mode confirmed pixel-unchanged | 2026-09-24 |
