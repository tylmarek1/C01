...
def test_invite_guest_success_and_shared_with_me(session_factory: sessionmaker) -> None:
    ...
    guest_token = client.post("/auth/login", json={"email": guest_email, "password": "supersecret"}).json()["access_token"]
    shared = client.get("/reservations/shared-with-me", headers={"Authorization": f"Bearer {guest_token}"})
    assert shared.status_code == 200
    assert len(shared.json()) == 1
    assert shared.json()[0]["id"] == created["id"]

    guest_notifications = client.get("/notifications", headers={"Authorization": f"Bearer {guest_token}"})
    assert any(n["title"].startswith("You've been added") for n in guest_notifications.json())
...
def test_cannot_invite_twice(session_factory: sessionmaker) -> None:
    ...
    first = client.post(f"/reservations/{created['id']}/guests", json={"email": guest_email}, headers=headers)
    assert first.status_code == 201
    second = client.post(f"/reservations/{created['id']}/guests", json={"email": guest_email}, headers=headers)
...
def test_remove_guest(session_factory: sessionmaker) -> None:
    ...
    guest = client.post(f"/reservations/{created['id']}/guests", json={"email": guest_email}, headers=headers).json()

    response = client.delete(f"/reservations/{created['id']}/guests/{guest['user']['id']}", headers=headers)
