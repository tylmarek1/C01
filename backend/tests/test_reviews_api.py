...
def seed_completed_reservation(session_factory: sessionmaker, user_id, court_name: str = "Tennis 1") -> tuple:
...
def test_cannot_review_twice(session_factory: sessionmaker) -> None:
    ...
    first = client.post("/reviews", json={"reservation_id": reservation_id, "rating": 5}, headers=headers)
    assert first.status_code == 201
    second = client.post("/reviews", json={"reservation_id": reservation_id, "rating": 3}, headers=headers)
