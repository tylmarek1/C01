...
def list_open_games(
    ...
    already_on = select(ReservationGuest.reservation_id).where(
    ...
    for reservation in reservations:
        ...
        if spots_left > 0:
            reservation.spots_left = (
                spots_left  # transient, read by OpenGameOut (from_attributes)
            )
...
def list_frequent_teammates(
    ...
    for teammate_id, count in rows:
        ...
        if user is not None:
            teammates.append(
                TeammateOut(user=UserOut.model_validate(user), games_together=count)
            )
    return teammates
...
def personal_calendar_feed(token: str, db: Session = Depends(get_db)) -> Response:
    ...
    if user is None:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND, "Invalid or revoked calendar token"
        )
    ...
    reservations = list(db.scalars(stmt))
    return Response(
        content=build_calendar_feed_ics(reservations), media_type="text/calendar"
    )


@router.post(
    "/series", response_model=ReservationSeriesOut, status_code=status.HTTP_201_CREATED
)
def create_reservation_series(
    ...
    # A series can request many more weeks than a player is allowed to hold
    # active at once — check_active_reservation_limit above only rejects the
    # request outright if they're already at the cap. Track remaining
    # capacity through the loop too, so a series that would blow past the
    # cap partway through degrades to a partial booking (like a genuine
    # slot conflict) instead of silently bypassing the cap altogether.
    active_limit = rules.max_active_reservations(current_user.role)
    ...
    for week in range(payload.weeks):
        ...
        if existing_active_count + len(booked) >= active_limit:
    ...
    if not booked:
        ...
        raise HTTPException(
            status.HTTP_409_CONFLICT,
            "Every week in that series conflicted with an existing booking",
        )
    ...
    return ReservationSeriesOut(
        series_id=series.id,
        requested_occurrences=payload.weeks,
        booked=booked,
...
def reschedule_reservation(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(
            status.HTTP_409_CONFLICT, "New slot is already taken"
        ) from exc
    db.refresh(reservation)
    return reservation
...
@router.post(
    ...
    response_model=ReservationGuestOut,
...
def invite_guest(
    ...
    if reservation.status not in (
        ...
        raise HTTPException(
            status.HTTP_409_CONFLICT, "Can't invite guests to a reservation that's over"
        )
    ...
    guest = ReservationGuest(
        reservation_id=reservation.id, user_id=invitee.id, invited_by_id=current_user.id
    )
    db.add(guest)
    try:
        db.flush()
        ...
        db.rollback()
        raise HTTPException(
            status.HTTP_409_CONFLICT, "That person is already invited"
        ) from exc
...
@router.delete(
    "/{reservation_id}/guests/{user_id}", status_code=status.HTTP_204_NO_CONTENT
)
def remove_guest(
    reservation_id: uuid.UUID,
    user_id: uuid.UUID,
    ...
    if guest is None:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND, "Guest not found on this reservation"
        )
    db.delete(guest)
    db.commit()
...
def set_open_to_join(
    ...
    if reservation.status != ReservationStatus.CONFIRMED:
        raise HTTPException(
            status.HTTP_409_CONFLICT,
            "Only a confirmed reservation can be opened up to other players",
        )
...
def request_to_join(
    ...
    if reservation is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Reservation not found")
    if (
        not reservation.open_to_join
        or reservation.status != ReservationStatus.CONFIRMED
    ):
        raise HTTPException(
            status.HTTP_409_CONFLICT, "This reservation isn't open for join requests"
        )
    if reservation.user_id == current_user.id:
        raise HTTPException(
            status.HTTP_409_CONFLICT, "You're already the host of this reservation"
        )
    ...
    if already_guest is not None:
        raise HTTPException(
            status.HTTP_409_CONFLICT, "You're already on this reservation"
        )
    ...
    join_request = JoinRequest(
        reservation_id=reservation.id, user_id=current_user.id, note=payload.note
    )
    db.add(join_request)
    try:
        db.flush()
        ...
        db.rollback()
        raise HTTPException(
            status.HTTP_409_CONFLICT, "You already requested to join this reservation"
        ) from exc
...
@router.post(
    "/{reservation_id}/join-requests/{request_id}/accept",
    response_model=ReservationGuestOut,
)
def accept_join_request(
    ...
    if join_request.status != JoinRequestStatus.PENDING:
        raise HTTPException(
            status.HTTP_409_CONFLICT, "This request was already answered"
        )
    if _guest_count(db, reservation.id) >= rules.MAX_GUESTS_PER_RESERVATION:
        raise HTTPException(
            status.HTTP_409_CONFLICT,
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(
            status.HTTP_409_CONFLICT, "That person is already on this reservation"
        ) from exc
...
@router.post(
    "/{reservation_id}/join-requests/{request_id}/decline",
    response_model=JoinRequestOut,
)
...
def split_reservation_cost(
    ...
    return ReservationSplit(
        ...
        participants=[
            ReservationSplitParticipant(
                user=UserOut.model_validate(u), share=per_person or 0.0
            )
