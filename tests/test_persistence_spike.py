...
def seed_court_and_user(session_factory: sessionmaker) -> tuple[Court, User]:
    with session_factory() as session:
        ...
        user = User(name="Alice Player", email="alice@example.com")
...
def test_db_rejects_overlapping_confirmed(session_factory: sessionmaker) -> None:
    ...
    with session_factory() as session:
        # Overlapping DRAFT is allowed; back-to-back CONFIRMED is allowed ([) ranges).
        session.add(reservation(court, user, at(19), at(20), ReservationStatus.DRAFT))
